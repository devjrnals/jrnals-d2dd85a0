...
export const ShareDialog = ({
  ...
  useEffect(() => {
    if (open && journalId) {
      loadCurrentShare();
      setLinkGenerated(false); // Reset when dialog opens
    }
  }, [open, journalId]);
  ...
  const loadCurrentShare = async () => {
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 is "not found"
        ...
        // Don't set the link here - only show it after Generate is clicked
        // SECURITY: Use share_id (random UUID) in URL, NOT journal_id
        // This prevents exposing predictable journal IDs in public URLs
        const baseUrl = window.location.origin;
        const link = `${baseUrl}/shared/${data.share_id}`;
        setShareLink(link);
        // Don't set linkGenerated here - user needs to click Generate to see it
      } else {
        // No share exists yet - link will be generated after saving
        setShareLink("");
      }
  ...
  const saveShareSettings = async () => {
    ...
    try {
      ...
      if (currentShare) {
        // Update existing share (keep existing share_id)
        ...
        // Updated existing share - show the link
        const baseUrl = window.location.origin;
        const link = `${baseUrl}/shared/${currentShare.share_id}`;
        setShareLink(link);
        setLinkGenerated(true); // Show the link section now
      } else {
        // Create new share - share_id is auto-generated by database
        const { data, error } = await (supabase as any)
          .from("journal_shares")
          .insert(shareData)
          .select()
        ...
        // Set the share link immediately using the new share_id
        if (data) {
          const baseUrl = window.location.origin;
          const link = `${baseUrl}/shared/${data.share_id}`;
          setShareLink(link);
      ...
      // Reload current share data to ensure UI is in sync
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          {/* Share Link - Only show after Generate is clicked */}
